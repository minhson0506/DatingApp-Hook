...
const RegisterForm = ({setFormToggle}) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Default full_name (data) for new user register
      const additionData = {
        deleted_hook: 0,
      ...
      data.email = `${data.username}@a.fi`; // autogenerated email for user
      const userData = await postUser(data);
      setInstruction(true); // condition to check if user is new (needs instruction) or not
      if (userData) {
